...
function icsToJson(data,reject,relationships,log=true) {
    const events = [];
	const lines = data.replace('\n ', '').replace(/^\s+|\s+$/g, '').split('\r\n'); // \r

        if (!validateIcs(lines)) {
            reject('Invalid ICS file');
            return;
        }
        ...
        lines.forEach(function (line) {
            switch (line) {
                ...
                case 'BEGIN:VEVENT':
                    parsedEvent = {};
                    parsedEvent.scope_ids = relationships['scope-ids'];
                    parsedEvent.separate_users = relationships['separate-users'];
                    break;
                case 'END:VEVENT':
                    if (!isParsingAlarm) {
                        events.push(parsedEvent);
                    } else {
                        if(log) logger.error('[icsToJson] Created invalid JSON, are all required fields present?');
                    }
                    break;
                case 'BEGIN:VALARM':
                    parsedAlarm = {};
                ...
                default:
                    if (isParsingAlarm) {
                        ...
                        lineToJson(line, parsedEvent,log);
                    }
                    break;
            }
        });

    let validationResult = validateJson(events, true, 'GET');
    if (validationResult === true) {
		return events;
    } else {
        reject(`Invalid ICS or missing JSON field: ${validationResult}`);
    }
}

function lineToJson(line, event,log) {
    const splitPosition = line.indexOf(':');

    if (splitPosition === -1) {
        if(log) logger.error('[icsToJson] Invalid line in ICS.');
        return;
    }
	
	//attributes maybe later as additional key field SUMMARY;LANGUAGE=de to summary_language:'de'
    const fieldName = (line.substr(0, splitPosition)).split(';')[0]; //.split(';')[0] to remove additional attributes
            ...
            for (let i = 0; i < repeatAttributes.length; i++) {
                ...
                switch (raName) {
                    ...
                    case 'WKST':
                        ...
                        break;
                    default:{
						if(log){ logger.error('[icsToJson] Invalid repeat attribute: ' + raName + ': ' + raValue); }
					}
                ...
                switch (realFieldName) {
                    ...
                    default:
                        if (realFieldName.startsWith('X-')) {
                           ...
                           if(log) logger.error('[icsToJson] Got unknown ICS field. Implement ' + fieldName);
                        }
                        break;
                }
            }
...
function lineToAlarmJson(line, alarm) {
    ...
    if (splitPosition === -1) {
        if(log) logger.error('[icsToJson] Invalid line in ICS (parsing alarm): ' + line);
        return;
    }
    ...
    switch (fieldName) {
        ...
        case 'SUMMARY':
            ...
            break;
        default:{
			if(log) logger.error('[icsToJson] Got unknown ICS field for alarm. Implement ' + fieldName);
            break;
		}
